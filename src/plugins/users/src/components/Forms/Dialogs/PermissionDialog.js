...
export class PermissionDialog extends Component {
  ...
  setPermissionName(props) {
    if (
      ...
      props.existingValues.user_permissions &&
      Array.isArray(props.existingValues.user_permissions)
    ) {
      const permissions = props.existingValues.user_permissions.map(entry => {
        return entry.name;
      });
      this.setState({
        permissionNames: permissions,
        prepopulatedValues: props.existingValues.user_permissions
      });
    }
  }
